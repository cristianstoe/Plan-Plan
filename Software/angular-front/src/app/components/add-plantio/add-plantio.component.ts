...
export class AddPlantioComponent implements OnInit {
  ...
  retrieveUsers(): void {
    this.mainService.getAll(this.usersUrl).subscribe(
      (data) => {
        this.users = data;
        this.plantio.userId = data[0].nome;
  ...
  retrieveProducts(): void {
    this.mainService.getAll(this.productsUrl).subscribe(
      (data) => {
        this.products = data;
        this.plantio.produtoId = data[0].nome;
  ...
  savePlantio(): void {
    ...
    this.mainService.create(data, this.baseUrl).subscribe(
      (response) => {
